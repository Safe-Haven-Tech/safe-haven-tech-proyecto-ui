...
export default function Profile() {
  ...
  const fetchUsuario = useCallback(async (nickname) => {
    try {
      const token = localStorage.getItem('token');
      const url = `${import.meta.env.VITE_API_URL}/api/usuarios/public/${encodeURIComponent(nickname.trim())}`;
    ...
    return (
      ...
      >
        ...
        >
          ...
          <small className="text-muted mt-3">Buscando: @{nickname}</small>
        </div>
      </div>
    );
  ...
  return (
    <div
      style={{
        ...
        display: 'flex',
        flexDirection: 'column',
        paddingTop: '90px', // espacio para navbar
