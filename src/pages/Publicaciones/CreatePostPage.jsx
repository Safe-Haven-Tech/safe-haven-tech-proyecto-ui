...
const ImagenesCarrusel = ({ previews }) => {
  ...
  const handlePrev = () =>
    setActual((prev) => (prev === 0 ? previews.length - 1 : prev - 1));
  const handleNext = () =>
    setActual((prev) => (prev === previews.length - 1 ? 0 : prev + 1));
    ...
    <div className={postDetailStyles.carruselContainer}>
      {previews.length > 1 && (
        <>
          <button
            className={
              postDetailStyles.carruselBtn +
              ' ' +
              postDetailStyles.carruselBtnLeft
            }
            onClick={handlePrev}
            type="button"
          >
          ...
          <button
            className={
              postDetailStyles.carruselBtn +
              ' ' +
              postDetailStyles.carruselBtnRight
            }
            onClick={handleNext}
            type="button"
          >
      ...
      {previews[actual].tipo === 'imagen' ? (
        <img
          src={previews[actual].url}
          alt="preview"
          className={postDetailStyles.carruselImg}
        />
      ) : (
        <video
          src={previews[actual].url}
        ...
        <div className={postDetailStyles.carruselIndicators}>
          {previews.map((_, idx) => (
            <span
              ...
              className={
                idx === actual
                  ? postDetailStyles.carruselIndicatorActive
                  : postDetailStyles.carruselIndicator
              }
  ...
  const handleSubmit = async (e) => {
    ...
    if (archivos.length === 0) {
      setModal({
        show: true,
        success: false,
        mensaje: 'Debes subir al menos una imagen o video.',
      });
      return;
    }
    setSubiendo(true);
    try {
      ...
      const resUpload = await fetch(
        `${import.meta.env.VITE_API_URL}/api/publicaciones/${publicacionId}/upload`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (!resUpload.ok) {
        throw new Error(
          dataUpload.error || 'Error al subir archivos multimedia'
        );
      }
    ...
    <div className={styles.crearPostPageGrid}>
                ...
                <div key={idx} className={styles.previewMiniItem}>
                  {prev.tipo === 'imagen' ? (
                    <img
                      src={prev.url}
                      alt="preview"
                      className={styles.previewMiniImg}
                    />
                  ) : (
      ...
      {modal.show && (
        <div className={styles.modalOverlay}>
          <div
            className={`${styles.modal} ${modal.success ? styles.modalSuccess : styles.modalError}`}
          >
