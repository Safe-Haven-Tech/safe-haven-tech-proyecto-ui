...
import {
  fetchPublicacionesPerfil,
  fetchPublicaciones,
} from '../../services/publicacionesService';

/* Acorta texto preservando palabras cuando es posible */
const acortarTexto = (texto = '', max = 100) =>
  texto.length > max ? texto.slice(0, max).trimEnd() + '…' : texto;

/*
  Genera un thumbnail de video en cliente.
*/
    ...
    <div className="container">
      <div className={styles.welcomeContent}>
        ...
        <p className={styles.welcomeDescription}>
          Descubre recursos, conecta con una comunidad de apoyo y encuentra las
          herramientas que necesitas para cuidar tu salud mental.
        </p>
        <div className={styles.ctaContainer}>
          <Link
            ...
            role="button"
            aria-label="Comenzar Autoevaluación"
          >
  ...
  useEffect(() => {
    ...
    const extractListFromResponse = (data) => {
      ...
      for (const key of possibleKeys) {
        ...
        if (data[key] && Array.isArray(data[key].usuarios))
          return data[key].usuarios;
      }
      if (data.data && Array.isArray(data.data.usuarios))
    ...
    const load = async () => {
      try {
        ...
        if (mountedRef.current)
          setExperts(Array.isArray(list) ? list.slice(0, 6) : []);
      } catch (err) {
        if (mountedRef.current)
          setError(err?.message || 'Error al cargar expertos');
      } finally {
    ...
    <section className="container my-5" aria-labelledby="experts-title">
      ...
      {!loading && experts.length === 0 && !error && (
        <div className="text-center text-muted">
          No se encontraron profesionales en este momento.
        </div>
      )}
      ...
      <div className={`${styles.expertsGrid} row`}>
        {experts.map((prof) => {
          const idOrNick =
            prof.nombreUsuario || prof.nickname || prof._id || prof.id;
          ...
          const especialidades =
            prof.infoProfesional?.especialidades || prof.especialidades || [];
          const specialty =
          ...
          const disponible =
            (prof.infoProfesional &&
              (prof.infoProfesional.disponible ??
                prof.infoProfesional?.disponible)) ??
            prof.disponible ??
            undefined;
          ...
          return (
            <div
              key={prof._id || prof.id || prof.nombreUsuario || prof.nickname}
              className="col-md-4 mb-4"
            >
              <div
                className={`${styles.expertCard} card h-100 shadow-sm border-0`}
              >
                ...
                <div className="card-body d-flex flex-column">
                  ...
                  >
                    ...
                    <div>
                      {disponible ? (
                        <span
                          className={`${styles.expertBadge} ${styles.expertAvailable}`}
                        >
                          Disponible
                        </span>
                      ) : (
                        <span className={styles.expertBadge}>
                          {disponible === false ? 'No disponible' : 'Consultar'}
                        </span>
                      )}
                    </div>
                  </div>
...
/* Multimedia preview que genera thumbnails para video bajo demanda */
const MultimediaPreview = ({ url }) => {
  const [thumb, setThumb] = useState('');
  const [generating, setGenerating] = useState(false);
  const elRef = useRef(null);
  const mountedRef = useRef(true);
  const revokedRef = useRef([]);
  ...
  useEffect(() => {
    ...
    const cargar = async () => {
      try {
        ...
        if (mountedRef.current) setPublicaciones(data.publicaciones || []);
      } catch (err) {
        if (mountedRef.current)
  ...
  return (
    <section className="container my-5" aria-labelledby="publicaciones-title">
      ...
      {!loading && publicacionesPreview.length === 0 && (
        <div className="text-center text-muted">
          No hay publicaciones disponibles. Sé el primero en compartir.
        </div>
      )}
          ...
          <div key={pub._id} className="col-md-4 mb-4">
            ...
            >
              <Link
                ...
                aria-label={`Ver publicación ${acortarTexto(pub.contenido || '', 40)}`}
              >
                <MultimediaPreview
                  url={
              ...
              <div className="card-body">
                ...
                >
                  <img
                    ...
                    alt={
                      pub.autorId?.anonimo
                        ? 'Avatar anónimo'
                        : `${pub.autorId?.nombreCompleto || pub.autorId?.nombreUsuario} avatar`
                    }
                    loading="lazy"
                  ...
                  <span
                    className="fw-bold"
                    style={{ color: '#603c7e', fontSize: '1rem' }}
                  >
                    {pub.autorId?.anonimo
                      ? 'Anónimo'
                ...
                <p
                  className="mt-2 mb-2"
                  style={{ fontSize: '1rem', color: '#334155', minHeight: 40 }}
                >
                ...
                <div className="d-flex align-items-center mt-1">
                  ...
                  <span
                    className="me-3"
                    style={{ color: '#603c7e', fontSize: '0.95rem' }}
                  >
    ...
    const cargar = async () => {
      ...
      try {
        ...
        const publicacionesForo = (data.publicaciones || [])
          ...
          .slice(0, 6);
        if (mountedRef.current) setForoPopulares(publicacionesForo);
      } catch (err) {
        if (mountedRef.current)
          setErrorForo(
            err?.message || 'Error al cargar publicaciones del foro'
          );
      } finally {
        if (mountedRef.current) setLoadingForo(false);
      }
    };
    cargar();
    return () => {
  ...
  return (
    <section className="container my-5" aria-labelledby="foro-title">
      ...
      <div className={styles.foroSectionContainer} role="list">
        ...
        {loadingForo && (
          <div className={styles.foroLoading}>
            Cargando publicaciones del foro...
          </div>
        )}
        {errorForo && (
          <div className={`alert alert-danger ${styles.foroError}`}>
            {errorForo}
          </div>
        )}
        ...
        {foroPopulares.map((pub, idx) => {
          const fullAuthor = pub.autorId?.anonimo
            ? 'Anónimo'
            : pub.autorId?.nombreCompleto ||
              pub.autorId?.nombreUsuario ||
              'Usuario';
          const displayAuthor = pub.autorId?.anonimo
            ? 'Anónimo'
            : acortarTexto(fullAuthor, 12);
          return (
            <Link
              key={pub._id}
              to={`/publicacion/${pub._id}`}
            ...
            >
              ...
              <span className={styles.foroAutor} title={fullAuthor}>
                ...
                />
                <span className={styles.foroAutorNombre}>{displayAuthor}</span>
              </span>
