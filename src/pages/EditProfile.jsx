...
const validarNombre = (nombre) => {
  ...
  if (nombre.trim().length < LIMITES.NOMBRE.MIN)
    return `El nombre completo debe tener al menos ${LIMITES.NOMBRE.MIN} caracteres`;
  if (nombre.trim().length > LIMITES.NOMBRE.MAX)
    return `El nombre completo no puede superar los ${LIMITES.NOMBRE.MAX} caracteres`;
  return null;
};
...
const validarGenero = (genero) => {
  ...
  if (genero.length > LIMITES.GENERO.MAX) 
    return `El género no puede superar los ${LIMITES.GENERO.MAX} caracteres`;
  return null;
};
...
const ModalConfirmacion = ({ mostrar, titulo, mensaje, onConfirmar, onCancelar }) => {
      ...
      <div className="modal-dialog modal-dialog-centered">
        <div className="modal-content">
          ...
          <div className="modal-footer">
            ...
            <button type="button" className="btn btn-success" onClick={onConfirmar}>
              Confirmar
            </button>
          </div>
        </div>
...
const AvatarUploader = React.memo(({ usuario, previsualizacionImagen, onImageSelected, onRemoveImage }) => {
      ...
      >
        {previsualizacionImagen ? (
          ...
          <img
            ...
            style={{ width: '100%', height: '100%', objectFit: 'cover' }}
            onError={(e) => {
              e.target.src = 'https://static.vecteezy.com/system/resources/thumbnails/003/337/584/small_2x/default-avatar-photo-placeholder-profile-icon-vector.jpg';
            }}
          />
              ) : (
                <img
...
const EliminarFotoLink = ({ usuario, eliminarFoto, onEliminarFoto, onCancelarEliminacion }) => {
...
function perfilReducer(state, action) {
  switch (action.type) {
    ...
    case 'LIMPIAR_IMAGEN':
      return { ...state, imagenSeleccionada: null, previsualizacionImagen: null };
    ...
    case 'SET_FOTO_ELIMINADA':
          return { 
            ...
            previsualizacionImagen: action.eliminada ? null : state.previsualizacionImagen
          };
    default:
      return state;
  }
  ...
  useEffect(() => {
    ...
    const fetchUsuario = async () => {
      try {
        ...
        console.error(err);
        dispatch({ type: 'SET_ERROR', error: 'Error al cargar los datos del usuario' });
  ...
  useEffect(() => {
    const validarNickname = async () => {
      ...
      try {
        ...
        const data = await res.json();
        dispatch({ type: 'SET_NICKNAME_DISPONIBLE', disponible: data.disponible });
      } catch (err) {
...
const handleGuardar = useCallback(async () => {
  ...
  try {
    ...
    if (!res.ok) {
      if (res.status === 409 || data.error?.toLowerCase().includes('nickname')) {
        dispatch({ type: 'SET_ERROR', error: 'El nickname ya está en uso. Por favor elige otro.' });
        dispatch({ type: 'SET_NICKNAME_DISPONIBLE', disponible: false });
      } else {
        throw new Error(data.detalles?.join?.(', ') || data.error || data.message || 'Error al actualizar el perfil');
      }
      return;
    }

    dispatch({ type: 'SET_MENSAJE', mensaje: 'Perfil actualizado correctamente' });
    ...
    dispatch({ 
      ...
      usuario: {
        ...state.usuario,
        ...data.usuario,
        fotoPerfil: state.fotoEliminada ? null : (data.usuario?.fotoPerfil || state.usuario?.fotoPerfil)
      } 
    });
  ...
  const handleConfirmacion = useCallback((confirmar) => {
  ...
  const handleImagenSeleccionada = useCallback((event) => {
    ...
    if (archivo) {
      ...
      if (!LIMITES.IMAGEN.TIPOS_PERMITIDOS.includes(archivo.type)) {
        dispatch({ type: 'SET_ERROR', error: 'Solo se permiten imágenes JPEG y PNG' });
        return;
      }
      
      // Validar tamaño
      if (archivo.size > LIMITES.IMAGEN.MAX_SIZE) {
        dispatch({ type: 'SET_ERROR', error: 'La imagen no puede superar los 5MB' });
        return;
      }
    ...
    ← Volver a mi perfil
        ...
        <div className="mb-3">
          <label className="form-label fw-semibold">
            Nombre completo *
            <span className="text-muted" style={{ fontSize: '0.8rem', marginLeft: '5px' }}>
          ...
          <input
            ...
            onChange={(e) => dispatch({ type: 'SET_CAMPO', campo: 'nombreCompleto', valor: e.target.value })}
            style={{
              borderRadius: '14px',
        ...
        <div className="mb-3">
          <label className="form-label fw-semibold">
            Nickname *
            <span className="text-muted" style={{ fontSize: '0.8rem', marginLeft: '5px' }}>
              (único, {LIMITES.NICKNAME.MIN}-{LIMITES.NICKNAME.MAX} caracteres, letras, números y _)
            </span>
          </label>
          ...
          <div className="position-relative">
            <input
              ...
              onChange={(e) => dispatch({ type: 'SET_CAMPO', campo: 'nombreUsuario', valor: e.target.value })}
              style={{
                borderRadius: '14px',
                padding: '0.6rem 0.75rem',
                boxShadow: 'inset 0 2px 6px rgba(0,0,0,0.05)',
                borderColor: !state.validandoNickname && state.nombreUsuario ? 
                  (state.nicknameDisponible ? '#198754' : '#dc3545') : '#ced4da',
                borderWidth: '1px',
                borderStyle: 'solid'
              }}
            />
            ...
            {!state.validandoNickname && state.nombreUsuario && state.nicknameDisponible && (
              <div className="position-absolute" style={{ right: '12px', top: '50%', transform: 'translateY(-50%)' }}>
            ...
            {!state.validandoNickname && state.nombreUsuario && !state.nicknameDisponible && (
              <div className="position-absolute" style={{ right: '12px', top: '50%', transform: 'translateY(-50%)' }}>
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" className="text-danger" viewBox="0 0 16 16">
          ...
          {state.validandoNickname && (
            <div className="text-primary small mt-2">
              <i className="bi bi-hourglass-split me-1"></i>Verificando disponibilidad...
            </div>
          )}
          {!state.validandoNickname && state.nombreUsuario && !state.nicknameDisponible && (
            <div className="text-danger small mt-2">
              <i className="bi bi-exclamation-circle me-1"></i>Este nickname ya está en uso. Por favor, elige otro.
            </div>
          )}
          {!state.validandoNickname && state.nombreUsuario && state.nicknameDisponible && (
            <div className="text-success small mt-2">
              <i className="bi bi-check-circle me-1"></i>¡Perfecto! Este nickname está disponible.
            </div>
          )}
          {state.nombreUsuario && state.nombreUsuario.length < LIMITES.NICKNAME.MIN && (
            <div className="text-warning small mt-2">
              <i className="bi bi-info-circle me-1"></i>El nickname debe tener al menos {LIMITES.NICKNAME.MIN} caracteres.
            </div>
          )}
        </div>
        ...
        <div className="mb-3">
          ...
          <input
            ...
            onChange={(e) => dispatch({ type: 'SET_CAMPO', campo: 'pronombres', valor: e.target.value })}
            style={{
              borderRadius: '14px',
              padding: '0.6rem 0.75rem',
              boxShadow: 'inset 0 2px 6px rgba(0,0,0,0.05)',
        ...
        <div className="mb-3">
          ...
          <textarea
            ...
            onChange={(e) => dispatch({ type: 'SET_CAMPO', campo: 'biografia', valor: e.target.value })}
            rows="3"
            style={{
              borderRadius: '14px',
              padding: '0.6rem 0.75rem',
        ...
        <button
          ...
          disabled={state.cargando || state.validandoNickname || !state.nicknameDisponible}
          style={{
            borderRadius: '14px',
            boxShadow: '0 6px 18px rgba(34,197,94,0.3)',
            transition: 'transform 0.2s, box-shadow 0.2s',
            opacity: (state.cargando || state.validandoNickname || !state.nicknameDisponible) ? 0.7 : 1
          }}
          onMouseEnter={(e) => {
      ...
      >
        ...
        <div className="d-flex align-items-center gap-3 mb-3">
          ...
          <div>
            <h5 className="mb-1">@{state.nombreUsuario || state.usuario?.nombreUsuario}</h5>
            <h6 className="mb-1">{state.nombreCompleto || state.usuario?.nombreCompleto}</h6>
        ...
        {state.biografia && (
          <div className="mt-3 p-3" style={{ backgroundColor: '#f8f9fa', borderRadius: '12px' }}>
        ...
        <div className="mt-3 text-center text-muted small">
          <i className="bi bi-eye"></i> Esta es una vista previa de cómo se verá tu perfil
        </div>
      </div>
      ...
      <ModalConfirmacion
        ...
        mensaje={
          <>
            <p>¿Estás seguro de que quieres cambiar tu nickname de <strong>@{state.usuario?.nombreUsuario}</strong> a <strong>@{state.nombreUsuario}</strong>?</p>
            <p className="text-muted small">Esta acción no se puede deshacer y tu antiguo nickname podrá ser usado por otros usuarios.</p>
          </>
        }
