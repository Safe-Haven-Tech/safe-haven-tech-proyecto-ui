...
const MessageInput = ({ onSend = async () => {} }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await onSend(text.trim(), files.map(f => f.file), esTemporal, expiraEn);
      setText('');
      setEsTemporal(false);
      setExpiraOpcion('24');
      ...
      <div className="flex-grow-1">
        ...
        <div className="d-flex align-items-center gap-2 mt-2">
          <div className="form-check">
            ...
            />
            <label className="form-check-label small" htmlFor="msg-temporal">Mensaje efímero</label>
          </div>
        ...
        {files.length > 0 && (
          <div className="mt-2 d-flex flex-wrap gap-2">
            {files.map((f, i) => (
              <div key={i} className="border rounded p-1 bg-white" style={{ width: 100 }}>
                {f.isImage ? (
                  <img src={f.previewUrl} alt={f.file.name} style={{ width: '100%', height: 60, objectFit: 'cover', borderRadius: 4 }} />
                ) : (
                  <div className="small text-truncate" style={{ height: 60 }}>{f.file.name}</div>
                )}
                <div className="d-flex justify-content-between align-items-center mt-1">
                  <small className="text-muted" style={{ fontSize: 11 }}>{formatBytes(f.file.size)}</small>
                  <button type="button" className="btn btn-sm btn-link p-0" onClick={() => removeFile(i)} aria-label={`Eliminar ${f.file.name}`}>✕</button>
                </div>
              </div>
            ))}
          </div>
