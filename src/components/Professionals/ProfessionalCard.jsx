...
function normalizeUser(raw = {}) {
  ...
  return {
    ...
    nombreCompleto:
      raw.nombreCompleto ||
      `${raw.nombres || ''} ${raw.apellidos || ''}`.trim() ||
      raw.nombreUsuario ||
      '',
    ...
    infoProfesional: {
      ...
      especialidades: Array.isArray(info.especialidades)
        ? info.especialidades
        : [],
      registroProfesional: info.registroProfesional || info.registro || '',
      institucionTitulo: info.institucionTitulo || info.institucion || '',
      añosExperiencia:
        info['añosExperiencia'] ??
        info.aniosExperiencia ??
        info.experienceYears ??
        null,
      disponible:
        typeof info.disponible === 'boolean'
          ? info.disponible
          : (info.available ?? true),
      modalidadesAtencion: Array.isArray(info.modalidadesAtencion)
        ? info.modalidadesAtencion
        : Array.isArray(info.modalidades)
          ? info.modalidades
          : [],
...
export default function ProfessionalCard({ user: rawUser }) {
  ...
  const disponible =
    typeof info.disponible === 'boolean' ? info.disponible : true;
  ...
  return (
    <article
      ...
      aria-labelledby={`prof-${user._id}-name`}
      role="article"
    >
      <div className={styles.cardTop}>
        ...
        <div className={styles.headerInfo}>
          <h3 id={`prof-${user._id}-name`} className={styles.name}>
            {name}
          </h3>
          ...
          <div className={styles.metaRow}>
            <span
              className={`${styles.badge} ${disponible ? styles.available : styles.unavailable}`}
            >
          ...
          <div className={styles.smallMeta} aria-label="Detalles profesionales">
            ...
            {institucion && (
              <span style={{ marginRight: 10 }}>
                <strong>Formación:</strong> {institucion}
              </span>
            )}
            {experiencia !== null && (
              <span>
                <strong>Experiencia:</strong> {experiencia}{' '}
                {experiencia === 1 ? 'año' : 'años'}
              </span>
            )}
          </div>
        </div>
      ...
      <p className={styles.bio} aria-label="Resumen profesional">
        {bio
          ? bio.length > 140
            ? bio.slice(0, 140) + '…'
            : bio
          : 'Información breve no disponible.'}
      </p>
      ...
      <div className={styles.footer}>
        <div className={styles.footerLeft}>
          <div className={styles.smallMeta}>
            ...
            {modalidades.join(', ') || 'Presencial • Online'}
          </div>
          <div className={styles.smallMeta}>
            <strong>Idiomas:</strong> {idiomas.join(', ') || 'Español'}
          </div>
        </div>
        ...
        <div className={styles.actions}>
          ...
          <a
            ...
            href={`/perfil/${user.nombreUsuario || user._id}`}
            rel="noopener noreferrer"
          >
            Ver perfil
          </a>
