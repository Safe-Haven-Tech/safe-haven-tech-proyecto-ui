...
export default function HeroCard({ onStart }) {
  ...
  const handleCTAClick = () => {
    ...
    const target = document.getElementById('postulacion-form') || document.querySelector('form');
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 260 }}>
          <h1 className={styles.title}>
            {usuario ? `Hola, ${displayName}` : 'Postúlate para nuestro listado de profesionales'}
          </h1>
          <p className={styles.lead}>
            Comparte tu formación y experiencia para que las personas que han sufrido violencia reciban atención segura y profesional.
        ...
        {/* <div style={{ width: 240, height: 160, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Illo className={styles.heroIllustration} />
        </div> */}
      </div>
    </section>
  );
